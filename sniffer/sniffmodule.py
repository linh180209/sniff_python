			...
			if statics_info[n][0] == array_f_new[i].id:				
				for j in range(8):
					if array_f_new[i].bytechangecount[j] > 0:
						...
						if frcount ==1:
							keyfrarray.append(array_f_new[i])
							print array_f_new[i].id,array_f_new[i].data,j,array_f_new[i].bytechangecount[j],array_f_new[i].bytechangecount
							datatemp.append(array_f_new[i].data[j])
							break
						else:
							if array_f_new[i].data[j] not in datatemp :
								print array_f_new[i].id,array_f_new[i].data,j,array_f_new[i].bytechangecount[j],array_f_new[i].bytechangecount
...
def logfrarray2file(array_f,frtype = "P",frcomment = ""):
	...
	for i in range(len(array_f)):
		vtmsg = vtlog.VTMessage(array_f[i].id,8,array_f[i].data,array_f[i].count,0.01,frtype,frcomment)
	...
	for i in range(len(frarray)):		
		...
		while(flags_comment != 0):
			...
			if c == "r":
				...
				vtmsg = vtlog.VTMessage(fr.id,8,fr.data,8,0.01,"K",c)
				vtmsgbuffer.append(vtmsg)
				flags_comment = 0
			else:
				flags_comment = 0
